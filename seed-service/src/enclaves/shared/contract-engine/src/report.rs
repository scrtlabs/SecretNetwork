...
use super::cert::{get_netscape_comment, IAS_REPORT_CA};
...
impl AttestationReport {
    ...
    pub fn from_cert(cert: &[u8]) -> Result<Self, Error> {
        ...
        let signing_cert = webpki::EndEntityCert::try_from(report.signing_cert.as_slice())
            .map_err(|_err| {
                error!("Failed to validate signature");
                Error::ReportParseError
            })?;

        let anchors = vec![webpki::TrustAnchor::try_from_cert_der(IAS_REPORT_CA).unwrap()];
