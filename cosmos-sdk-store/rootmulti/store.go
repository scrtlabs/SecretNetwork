...
func (rs *Store) WorkingHash() []byte {
	...
	json, err := json.MarshalIndent(storeInfos, " ", "    ")
	if err != nil {
		fmt.Printf("ERR JSON MARSHAL INDENT: %+v\n", err)
	}
	fmt.Printf("STOREINFOS: %+v\n", string(json))
	fmt.Printf("CALCULATED APP HASH: %X\n", types.CommitInfo{StoreInfos: storeInfos}.Hash())
	...
	for key, store := range rs.stores {
		...
		case types.StoreTypeIAVL:
			...
			if err != nil {
				...
				rs.logger.Error("[*] Cache for", "module store", key.Name(), "error", err.Error(), "By-passing...")
				continue
