    ...
    match msg {
        ...
        ExecuteMsg::ExecChainOf2 {
            ...
            msg: CosmosMsg::Wasm(WasmMsg::Execute {
                ...
                msg: Binary::from(r#"{"exec_chain":{}}"#.as_bytes().to_vec()),
                funds: vec![],
            }),
            gas_limit: Some(10000000_u64),
            reply_on: ReplyOn::Always,
        })),
        ExecuteMsg::ExecChain {} => Ok(Response::new().set_data(to_binary("Lemon")?)),
        ExecuteMsg::InitV10 {
            counter,
            code_id,
            code_hash,
...
pub fn reply(deps: DepsMut, env: Env, reply: Reply) -> StdResult<Response> {
    match (reply.id, reply.result) {
        ...
        (4444, SubMsgResult::Ok(_)) => Ok(Response::new().set_data(to_binary("4444")?)),
        (4444, SubMsgResult::Err(_)) => Err(StdError::generic_err(format!("Unreachable",))),
        (4445, SubMsgResult::Ok(_)) => Ok(Response::new().set_data(to_binary("4444")?)),
        (4445, SubMsgResult::Err(_)) => Err(StdError::generic_err(format!("Unreachable",))),
