...
import (
	...
	"github.com/scrtlabs/SecretNetwork/rpc/backend/mocks"
	ethrpc "github.com/scrtlabs/SecretNetwork/rpc/types"
	"github.com/scrtlabs/SecretNetwork/tests"
...
func (suite *BackendTestSuite) TestGetBlockByNumber() {
	...
	testCases := []struct {
	...
	}{
		{
			...
			true,
			sdk.NewInt(1).BigInt(),
			sdk.AccAddress(tests.RandomEthAddress().Bytes()),
			nil,
			nil,
		...
		suite.Run(fmt.Sprintf("Case %s", tc.name), func() {
			...
			if tc.expTxs {
				if tc.fullTx {
					...
					ethRPCTxs = []interface{}{common.HexToHash(msgHandleTx.Hash)}
				}
			}
...
func (suite *BackendTestSuite) TestEthMsgsFromTendermintBlock() {
	...
	testCases := []struct {
		...
		},
		{
			...
			[]*evmtypes.MsgHandleTx{msgHandleTx},
		},
		{
			"pass",
			&tmrpctypes.ResultBlock{
	...
	testCases := []struct {
		...
		},
		{
			...
			ethtypes.NewBlock(
				...
				[]*ethtypes.Transaction{msgHandleTx.AsTransaction()},
				nil,
				nil,
