FROM ubuntu:noble AS runtime_base
...
ARG PSW_VERSION=2.25.100.3-noble1
ARG OS_REVESION=noble1
...
RUN apt-get update && \
    ...
    add-apt-repository "deb https://download.01.org/intel-sgx/sgx_repo/ubuntu noble main" && \
    apt-get update && \
    apt-get install -y \
...
#
ADD https://download.01.org/intel-sgx/sgx-linux/${SDK_VERSION}/distro/ubuntu24.04-server/sgx_linux_x64_sdk_${SGX_VERSION}.bin ./sgx/
