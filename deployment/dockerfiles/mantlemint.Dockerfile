...
COPY deployment/docker/mm/mantlemint.sh .

RUN chmod +x /usr/bin/secretd
RUN chmod +x mantlemint.sh
...
RUN git clone https://github.com/scrtlabs/mantlemint.git

WORKDIR mantlemint

RUN FEATURES_U=query-node SGX_MODE=HW go build -mod=readonly -o build/mantlemint ./sync.go
...
ENTRYPOINT ["/bin/bash", "mantlemint.sh"]
