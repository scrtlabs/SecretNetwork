...
func (c *EcallClient) FetchBlockTraces(height int64) ([]*ExecutionTrace, error) {
	...
	if len(traces) > 0 {
		for _, t := range traces {
			fmt.Printf("[EcallClient] Fetched trace: height=%d index=%d ops=%d resultLen=%d gasUsed=%d hasError=%v\n",
				height, t.Index, len(t.Ops), len(t.Result), t.GasUsed, t.HasError)
		}
