...
//export cGet
func cGet(ptr *C.db_t, gasMeter *C.gas_meter_t, usedGas *u64, key C.Buffer, val *C.Buffer, errOut *C.Buffer) (ret C.GoResult) { //nolint:all
...
//export cSet
func cSet(ptr *C.db_t, gasMeter *C.gas_meter_t, usedGas *C.uint64_t, key C.Buffer, val C.Buffer, errOut *C.Buffer) (ret C.GoResult) { //nolint:all
	defer recoverPanic(&ret)
...
//export cDelete
func cDelete(ptr *C.db_t, gasMeter *C.gas_meter_t, usedGas *C.uint64_t, key C.Buffer, errOut *C.Buffer) (ret C.GoResult) { //nolint:all
	defer recoverPanic(&ret)
	if ptr == nil || gasMeter == nil || usedGas == nil {
		// we received an invalid pointer
		return C.GoResult_BadArgument
...
//export cScan
func cScan(ptr *C.db_t, gasMeter *C.gas_meter_t, usedGas *C.uint64_t, start C.Buffer, end C.Buffer, order i32, out *C.GoIter, errOut *C.Buffer) (ret C.GoResult) { //nolint:all
	defer recoverPanic(&ret)
...
//export cNext
func cNext(ref C.iterator_t, gasMeter *C.gas_meter_t, usedGas *C.uint64_t, key *C.Buffer, val *C.Buffer, errOut *C.Buffer) (ret C.GoResult) { //nolint:all
