...
- RPC address of an already active node. You can use `bootstrap.pub.testnet2.enigma.co:26657`, or any other node that exposes RPC services.
...
```bash
wget https://github.com/enigmampc/SecretNetwork/releases/download/v0.8.0/secretnetwork_0.8.0_amd64.deb
```
...
```bash
sudo dpkg -i secretnetwork_0.8.0_amd64.deb
```
...
```bash
secretd init <MONIKER> --chain-id enigma-pub-testnet-3
```

### 4. Download a copy of the Genesis Block file: `genesis.json`

```bash
wget -O ~/.secretd/config/genesis.json "https://github.com/enigmampc/SecretNetwork/releases/download/v0.8.0/genesis.json"
```
...
```bash
perl -i -pe 's/persistent_peers = ""/persistent_peers = "115aa0a629f5d70dd1d464bc7e42799e00f4edae\@bootstrap.pub.testnet2.enigma.co:26656"/' ~/.secretd/config/config.toml
```
