...
func (node *Node) hashWithCount(version int64) []byte {
	if node == nil {
		fmt.Printf("NODE IS NULL, HASH IS %+v\n", sha256.New().Sum(nil))
		return sha256.New().Sum(nil)
	}
	if node.hash != nil {
		fmt.Printf("NODE HASH IS NOT NULL: %+v\n", node.hash)
		return node.hash
	}
	...
	fmt.Printf("FINAL NODE HASH IS: %+v\n", node.hash)
	return node.hash
}
...
func (node *Node) writeHashBytes(w io.Writer, version int64) error {
	...
	fmt.Printf("WRITING HASH:\nHEIGHT: %d\nSIZE: %d\nVERSION: %d\n", node.subtreeHeight, node.size, version)
	...
	if node.isLeaf() {
		...
		fmt.Printf("LEFT CHILD HASH: %+v\nRIGHT CHILD HASH: %+v\n", node.leftNode.hash, node.rightNode.hash)
	}
