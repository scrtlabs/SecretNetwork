...
import (
	...
	"github.com/scrtlabs/SecretNetwork/crypto/deoxys"
)
...
const (
	// TypeMsgHandleTx defines the type string of an Ethereum transaction
...
func newMsgHandleTx(
	...
	if to != nil {
		toAddr = to.Hex()
		if len(input) > 0 && len(privateKey) == 32 {
...
func (msg *MsgHandleTx) FromEthereumTx(tx *ethtypes.Transaction) error {
...
// Route returns the route value of an MsgHandleTx.
func (msg MsgHandleTx) Route() string { return RouterKey }

// Type returns the type value of an MsgHandleTx.
func (msg MsgHandleTx) Type() string { return TypeMsgHandleTx }

// ValidateBasic implements the sdk.Msg interface. It performs basic validation
// checks of a Transaction. If returns an error if validation fails.
func (msg MsgHandleTx) ValidateBasic() error {
	if msg.From != "" {
...
func (msg *MsgHandleTx) GetMsgs() []sdk.Msg {
	return []sdk.Msg{msg}
}
...
func (msg *MsgHandleTx) GetSigners() []sdk.AccAddress {
...
func (msg MsgHandleTx) GetSignBytes() []byte {
...
func (msg *MsgHandleTx) Sign(ethSigner ethtypes.Signer, keyringSigner keyring.Signer) error {
	from := msg.GetFrom()
	if from.Empty() {
	...
	// TODO: find out what sign mode to use
	sig, _, err := keyringSigner.SignByAddress(from, txHash.Bytes(), txsigning.SignMode_SIGN_MODE_LEGACY_AMINO_JSON)
	if err != nil {
		return err
	}
...
func (msg MsgHandleTx) GetGas() uint64 {
	txData, err := UnpackTxData(msg.Data)
	if err != nil {
		return 0
	}
...
func (msg MsgHandleTx) GetFee() *big.Int {
	txData, err := UnpackTxData(msg.Data)
	if err != nil {
		return nil
	}
...
func (msg MsgHandleTx) GetEffectiveFee(baseFee *big.Int) *big.Int {
	txData, err := UnpackTxData(msg.Data)
	if err != nil {
		return nil
	}
...
func (msg *MsgHandleTx) GetFrom() sdk.AccAddress {
	if msg.From == "" {
		return nil
	}
...
func (msg MsgHandleTx) AsTransaction() *ethtypes.Transaction {
	txData, err := UnpackTxData(msg.Data)
	if err != nil {
		return nil
	}
...
func (msg MsgHandleTx) AsMessage(signer ethtypes.Signer, baseFee *big.Int) (core.Message, error) {
	return msg.AsTransaction().AsMessage(signer, baseFee)
}

// GetSender extracts the sender address from the signature values using the latest signer for the given chainID.
func (msg *MsgHandleTx) GetSender(chainID *big.Int) (common.Address, error) {
...
func (msg MsgHandleTx) UnpackInterfaces(unpacker codectypes.AnyUnpacker) error {
	return unpacker.UnpackAny(msg.Data, new(TxData))
}

// UnmarshalBinary decodes the canonical encoding of transactions.
func (msg *MsgHandleTx) UnmarshalBinary(b []byte) error {
...
func (msg *MsgHandleTx) BuildTx(b client.TxBuilder, evmDenom string) (signing.Tx, error) {
