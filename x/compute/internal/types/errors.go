...
var (
	...
	ErrInstantiateFailed = sdkErrors.Register(DefaultCodespace, 3, "instantiate contract failed")

	// ErrExecuteFailed error for rust execution contract failure
	ErrExecuteFailed = sdkErrors.Register(DefaultCodespace, 4, "execute contract failed")
	...
	ErrQueryFailed = sdkErrors.Register(DefaultCodespace, 8, "query contract failed")

	// ErrInvalidMsg error when we cannot process the error returned from the contract
	ErrInvalidMsg = sdkErrors.Register(DefaultCodespace, 9, "invalid CosmosMsg from the contract")

	// ErrMigrationFailed error for rust execution contract failure
	ErrMigrationFailed = sdkErrors.Register(DefaultCodespace, 10, "migrate contract failed")
)
