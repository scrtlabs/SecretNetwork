...
func TestMultipleSigners(t *testing.T) {
	//
	t.SkipNow() // skipping till multisig is fixed
	ctx, keeper, codeID, codeHash, walletA, privKeyA, walletB, privKeyB := setupTest(t, "./testdata/test-contract/contract.wasm", sdk.Coins{})
...
func TestWrongSigner(t *testing.T) {
	t.SkipNow() // skipping till multisig is fixed
	ctx, keeper, codeID, _, walletA, _, walletB, privKeyB := setupTest(t, "./testdata/test-contract/contract.wasm", sdk.Coins{})
...
func TestMultiSig(t *testing.T) {
	t.SkipNow() // skipping till multisig is fixed
	ctx, keeper, codeID, codeHash, _, _, _, _ := setupTest(t, "./testdata/test-contract/contract.wasm", sdk.Coins{})
...
func TestMultiSigThreshold(t *testing.T) {
	...
	for i := 0; i < 5; i++ {
		for j := 0; j <= i; j++ {
			...
			// Reset wasm events
			ctx, keeper, codeID, _, _, _, _, _ = setupTest(t, "./testdata/test-contract/contract.wasm", sdk.Coins{})
		}
	}
}
...
func TestMultiSigCallbacks(t *testing.T) {
	t.SkipNow() // skipping till multisig is fixed
	ctx, keeper, codeID, codeHash, walletA, privKeyA, _, _ := setupTest(t, "./testdata/test-contract/contract.wasm", sdk.Coins{})
...
func TestMultiSigInMultiSig(t *testing.T) {
	t.SkipNow() // skipping till multisig is fixed
	ctx, keeper, codeID, codeHash, _, privKeyA, _, privKeyB := setupTest(t, "./testdata/test-contract/contract.wasm", sdk.Coins{})
