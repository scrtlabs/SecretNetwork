...
var testContracts = []TestContract{
	{
		...
		IsCosmWasmV1:    false,
		WasmFilePath:    filepath.Join(".", contractPath, "contract.wasm"),
	}, {
		CosmWasmVersion: "v1",
		IsCosmWasmV1:    true,
		WasmFilePath:    filepath.Join(".", contractPath, "v1-contract.wasm"),
	},
}
...
func TestContractTryToSendFundsFromSomeoneElse(t *testing.T) {
	ctx, keeper, codeID, _, walletA, privKeyA, _, _ := setupTest(t, testContracts[0].WasmFilePath, sdk.NewCoins())
...
func TestWasmTooHighInitialMemoryRuntimeFail(t *testing.T) {
	ctx, keeper, codeID, _, walletA, privKeyA, _, _ := setupTest(t, filepath.Join(".", contractPath, "/too-high-initial-memory.wasm"), sdk.NewCoins())
...
func TestWasmTooHighInitialMemoryStaticFail(t *testing.T) {
	...
	wasmCode, err := os.ReadFile(filepath.Join(".", contractPath, "/static-too-high-initial-memory.wasm"))
	require.NoError(t, err)
...
func TestWasmWithFloatingPoints(t *testing.T) {
	for _, testContract := range testContracts {
		t.Run(testContract.CosmWasmVersion, func(t *testing.T) {
			ctx, keeper, codeID, _, walletA, privKeyA, _, _ := setupTest(t, filepath.Join(".", contractPath, "/contract_with_floats.wasm"), sdk.NewCoins())
...
func TestEncryptedAndPlaintextLogs(t *testing.T) {
	ctx, keeper, codeID, _, walletA, privKeyA, _, _ := setupTest(t, filepath.Join(".", contractPath, "/plaintext-logs.wasm"), sdk.NewCoins())
...
func TestV1EndpointsSanity(t *testing.T) {
	ctx, keeper, codeID, _, walletA, privKeyA, _, _ := setupTest(t, testContracts[1].WasmFilePath, sdk.NewCoins())
...
func TestV010BankMsgSendFrom(t *testing.T) {
	for _, callType := range []string{"init", "exec"} {
		t.Run(callType, func(t *testing.T) {
			ctx, keeper, codeID, _, walletA, privKeyA, walletB, _ := setupTest(t, testContracts[0].WasmFilePath, sdk.NewCoins())
...
func TestV1InitV010ContractWithReply(t *testing.T) {
	...
	wasmCode, err := os.ReadFile(testContracts[0].WasmFilePath)
	require.NoError(t, err)
...
func TestV010InitV1ContractFromInitWithOkResponse(t *testing.T) {
	ctx, keeper, codeID, codeHash, walletA, privKeyA, _, _ := setupTest(t, testContracts[1].WasmFilePath, sdk.NewCoins())
...
func TestV010SendsLogsMixedWithV1(t *testing.T) {
	ctx, keeper, codeID, v1CodeHash, walletA, privKeyA, _, _ := setupTest(t, testContracts[1].WasmFilePath, sdk.NewCoins())
