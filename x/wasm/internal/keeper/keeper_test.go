...
func TestCreate(t *testing.T) {
	...
	contractID, err := keeper.Create(ctx, creator, wasmCode, "https://github.com/enigmampc/enigmachain/blob/master/x/wasm/testdata/escrow.wasm", "cosmwasm-opt:0.5.2")
	require.NoError(t, err)
...
func TestCreateWithGzippedPayload(t *testing.T) {
	...
	contractID, err := keeper.Create(ctx, creator, wasmCode, "https://github.com/enigmampc/enigmachain/blob/master/x/wasm/testdata/escrow.wasm", "")
	require.NoError(t, err)
	require.Equal(t, uint64(1), contractID)
	// and verify content
	storedCode, err := keeper.GetByteCode(ctx, contractID)
